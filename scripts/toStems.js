...
function processTxtFiles() {
    ...
    function save(words, outputPath){
        const toSave = Array.from(words).join("\n")
        fs.writeFileSync(outputPath, toSave);
    }
    ...
    save(vulgarWords, 'data/vulgarWords_PL.txt')
}

function processCSVFile() {
    inputPath = 'data/emotionalWords.csv'
    outputPath = 'data/emotionalStems.csv'
    var myFile = fs.createWriteStream(outputPath);

    let stemsValuesRows = [['word','meanAnger']]

    const stream = csv.parseFile(inputPath, { headers: true })
    .on('error', error => console.error(error))
    .on('data', row => stemsValuesRows.push([getStem(row.word), row.meanAnger]))
    .on('end', (rowCount) => {
        console.log(`Parsed ${rowCount} rows`)
        csv.write(stemsValuesRows).pipe(myFile);
    });
